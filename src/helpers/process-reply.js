const REPLY = `<div class="message-badge">
...
const REG_REPLY = /(\[reply mid:([0-9]+) to:([0-9]+)\])/g;
const REG_TO_ID = /(?<=to:)([0-9]+)/g;

export function Reply(content){
    content.replace(regExpReply,
        function(matchs){
            let toId = matchs.match(REG_TO_ID);
            console.log(toId);
        });
}